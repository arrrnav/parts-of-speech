...
run_experiments.py
------------------
Full experiment pipeline for CRF POS tagging on Penn Treebank.

Experiments
-----------
1. Feature-set ablation  (FS1 → FS4)
2. Training set size curve (5% → 100%)
3. L2 regularisation sweep (c2 grid, evaluated on dev set)
4. Per-tag analysis + confusion matrix on best model

Usage
-----
    python run_experiments.py

All figures are saved to pos_crf/figures/.
Results summary is printed to stdout.

Note: Each CRF fit runs L-BFGS for max_iterations=80 iterations.
For fully converged models, increase max_iterations (expect ~2-3x longer).
"""
...
all_sents   = [s for s in treebank.tagged_sents()
               if any(t != '-NONE-' for _, t in s)]
n           = len(all_sents)
n_train     = int(0.80 * n)
n_dev       = int(0.10 * n)
...
MAX_ITER = 80   # increase for fully converged models
...
# ── Experiment 3: L2 regularisation sweep ──────────────────────────────────
...
# ── Experiment 4: Best model — full evaluation ─────────────────────────────
